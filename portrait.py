...
def create_flashcards_pdf(words, name, pdf_filename, font_path):
    ...
    width, height = A4  # Получаем размеры A4
    margin = 40  # Отступы от краев страницы
    cards_per_row, cards_per_col = 3, 6  # Количество карточек в строке и колонке
    ...
    for i, word in enumerate(words):
        ...
        c.setFillColor(black)  # Цвет текста (сверху карточки)
        c.setFont("JapaneseFont", 8)  # Мелкий текст сверху карточки
        c.drawCentredString(x_pos + card_width / 1.3, y_pos + card_height / 1.2, f"第{name}課")
    ...
    for page_positions in kanji_positions_per_page:
        for (x_pos, y_pos, word) in page_positions:
            ...
            c.setFillColor(black)  # Цвет текста
            c.setFont("JapaneseFont", 20)  # Устанавливаем шрифт для чтения
            c.drawCentredString(mirrored_x_pos + card_width / 2, y_pos + card_height / 2, word["reading"])

            c.setFillColor(grey)  # Цвет текста
            ...
            for line in translation_lines:
                ...
                c.setFillColor(black)  # Цвет текста
                c.drawCentredString(mirrored_x_pos + card_width / 4.7, y_pos + card_height / 1.2, f"第{name}課")
